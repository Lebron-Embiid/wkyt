		...
		methods:{
			...
			initData(){
				...
				if(mode=="region"){
					...
					// 	},
					// })
					api.get('index.php?act=login&op=address_list', {}).then(res => {
						console.log(res.address_list);
						var item = res.address_list;
						
						for (let i = 0; i < item.length; i++) {
							provinces_arr.push({
								id: item[i].area_id,
								label: item[i].area_name
							})
						}
						...
						for (let i = 0; i < item.length; i++) {
							arr1 = [];
							// console.log(item[i].city_list);
							for (let j = 0; j < item[i].city_list.length; j++) {
								// console.log(item[i].city_list[j].area_parent_id);
								if(item[i].area_id == item[i].city_list[j].area_parent_id){
									arr1.push({
										id: item[i].city_list[j].area_id,
										label: item[i].city_list[j].area_name
									})
								}
							}
							// console.log(arr1);
						...
						for (let i = 0; i < item.length; i++) {
							...
							for (let j = 0; j < item[i].city_list.length; j++) {
								...
								// if(){
									for(let x = 0;x < item[i].city_list[j].area_list.length; x++){
										// console.log(item[i].list[j].list);
										if(item[i].area_id == item[i].city_list[j].area_parent_id && item[i].city_list[j].area_id == item[i].city_list[j].area_list[x].area_parent_id){
											// console.log(item[i].list[j].list);
											arr3.push({
												id: item[i].city_list[j].area_list[x].area_id,
												label: item[i].city_list[j].area_list[x].area_name
											})
										}
									}
								arr2.push(arr3)
